...
export type SettingsToggleScope = 'privacy' | 'notification' | 'local'
...
export type LocalSettingsKey = 'enableSound' | 'enableDesktopNotify'

export type SettingsToggleKey = PrivacySettingsKey | NotificationSettingsKey | LocalSettingsKey
...
export const settingsRegistry: ISettingsSection[] = [
  ...
  },
  {
    ...
    keyboard: [
      ...
      { id: 'search', label: '搜索', desc: '打开全局搜索窗口' },
  ...
  },
  {
    ...
    type: 'toggle-group',
    fields: [
      { scope: 'local', key: 'enableSound', label: '消息提示音', desc: '收到新消息时播放提示音（仅本机）' },
      { scope: 'local', key: 'enableDesktopNotify', label: '桌面通知', desc: '在系统通知栏显示消息提醒（仅本机）' },
