<template>
  <BeaverDialog v-model="visible" title="创建群聊" width="720px" @close="handleClose">
    <div class="group-creation">
      <!-- 左侧 -->
      <div class="group-creation-left">
        <div class="group-name-input">
          <label>群聊名称</label>
          <input v-model="groupName" type="text" class="input-field" placeholder="请输入群聊名称">
        </div>
      ...
      <div class="group-creation-right">
        <div class="search-friends">
          ...
          <input v-model="searchText" type="text" class="search-input" placeholder="搜索好友">
        </div>
    ...
    <template #footer>
      ...
      <button class="confirm-button" @click="handleCreateGroup">
        创建
      </button>
    </template>
  </BeaverDialog>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const handleAvatarError = (_e: Event) => {
      const _target = _e.target as HTMLImageElement
    }
