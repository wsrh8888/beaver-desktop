...
class ChatUserConversation extends BaseService {
  ...
  // 更新用户的已读游标
   async updateReadSeq(userId: string, conversationId: string, readSeq: number) {
  ...
  // 更新会话设置（置顶、免打扰等）
   async updateSettings(userId: string, conversationId: string, settings: {
    isPinned?: boolean
    isMuted?: boolean
    isHidden?: boolean
  }) {
  ...
  // 获取用户的基础会话数据（仅数据访问，不含业务逻辑）
   async getUserConversations(req: { userId: string, params: { page?: number, limit?: number, offset?: number } }) {
  ...
  // 根据会话ID列表批量获取用户的会话设置
   async getUserConversationsByIds(userId: string, conversationIds: string[]) {
  ...
  // 根据会话ID获取会话信息
   async getConversationInfo(header: any, params: any) {
  ...
  // 按版本范围获取会话设置（用于数据同步）
   async getChatConversationsByVerRange(header: any, params: any) {
    const userId = String(header.userId)
