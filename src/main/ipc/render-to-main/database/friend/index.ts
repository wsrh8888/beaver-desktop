...
export class FriendHandler {
   ...
   * 处理用户相关的数据库命令
   */
  static async handle(_event: Electron.IpcMainInvokeEvent, command: DataFriendCommand, data: any, header: any = {}): Promise<any> {
    ...
    switch (command) {
      case DataFriendCommand.GET_FRIENDS:
        ...
        return await FriendService.getFriends(header, data)
