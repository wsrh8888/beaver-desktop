  ...
  private async syncMessagesForConversations(conversationsWithSeq: Array<{ conversationId: string, serverSeq: number }>) {
    try {
      ...
      for (const { conversationId, serverSeq } of conversationsWithSeq) {
        // 获取本地消息同步状态
        const localSyncStatus = await dBServiceChatSyncStatus.getMessageSyncStatus({ conversationId }) as any
      ...
      while (currentSeq <= toSeq) {
        ...
        if (response.result.messages && response.result.messages.length > 0) {
          const messages = response.result.messages.map(msg => ({
            ...
            msgType: msg.msgType,
            targetMessageId: (msg as any).targetMessageId || null, // 引用消息ID（撤回/删除等）
          ...
          await dBServiceMessage.batchCreate({ messages: messages as any })

          // 清除正在发送消息的计时器 (ACK 确认)
          const syncedIds = messages.map(m => m.messageId).filter(id => id) as string[]
