...
import type { IUserSettings, KeyboardActionId } from '../../../../common/type/settings'
import { SettingsCommand } from '../../../../common/type/ipc/command'
import { Logger, registerKeyboardBinding, store } from '@beaver-im/beaver/main'
import { getUserSettingsApi } from '../../../api/user'
...
class SettingsHandler {
  async init() {
    ...
    ;(Object.keys(res.result.keyboard) as KeyboardActionId[]).forEach((actionId) => {
      registerKeyboardBinding(actionId, res.result!.keyboard[actionId])
    })
  }
  ...
  async handle(
    ...
    switch (command) {
      ...
      case SettingsCommand.SETTINGS_GET:
        return store.get('settings') as IUserSettings | undefined
