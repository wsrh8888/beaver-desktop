...
/**
 * @beaver-im/app-call 发版构建（两层）：
 * 1. main     — 主进程：activate / application / manifest（对等依赖 external）
 * 2. renderer — 渲染进程：call + call-incoming 双窗口挂载入口 + HTML 壳（自包含，loadFile 可用）
 *
 * 产物：
 *   dist/main.js
 *   dist/call.js (+css / assets)
 *   dist/call-incoming.js (+css / assets)
 *   dist/call.html
 *   dist/call-incoming.html
 */
  ...
  if (mode === 'renderer') {
    return {
      ...
      build: {
        ...
        rollupOptions: {
          input: {
            call: path.resolve(root, 'call.html'),
            'call-incoming': path.resolve(root, 'call-incoming.html'),
          },
          output: {
